...
from utils.dataloader import FacenetDataset, ArcfaceDataset, dataset_collate_arc, \
    dataset_collate_face
from utils.epochTrain import epochTrain_facenet, epochTrain_arcface
...
def train_facenet(config):
    ...
    collate_fn = dataset_collate_face
    ...
    for epoch in range(config.startEpoch, config.endEpoch):
        ...
        epochTrain_facenet(model_train, model, loss_history, loss, optimizer, epoch, epoch_step, epoch_step_val,
...
def train_arcface(config):
    ...
    num_classes = get_num_classes(config.dataPath)
    # 加载模型
    model = ArcFace(backbone=config.backbone, num_classes=num_classes)
    # 加载权重
    if config.weightPath != '':
        if flag == 0:
            print('Load weights {}.'.format(config.weightPath))
    ...
    shuffle = True
    batchSize = config.batchSize // 3
    collate_fn = dataset_collate_arc
    gen = DataLoader(train_dataset, shuffle=shuffle, batch_size=batchSize,
                     num_workers=config.numWorkers,
                     pin_memory=True,
                     drop_last=True, collate_fn=collate_fn, sampler=train_sampler)
    ...
    for epoch in range(config.startEpoch, config.endEpoch):
        set_lr(optimizer, lr_func, epoch)
        epochTrain_arcface(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val,
                           gen, gen_val, config.endEpoch, config.batchSize // 3,
                           scaler, config.savePeriod, 'logs', flag)
    # 训练结束
    if flag == 0:
