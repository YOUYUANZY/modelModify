...
def epochTrain_facenet(model_train, model, loss_history, loss, optimizer, epoch, epoch_step, epoch_step_val, gen,
                       gen_val,
    ...
    if flag == 0:
        ...
        if (epoch + 1) % save_period == 0 or epoch + 1 == endEpoch:
            torch.save(model.state_dict(), os.path.join(save_dir, 'facenet_%03d-l%.3f-vl%.3f.pth' % (
                (epoch + 1), (total_triple_loss + total_CE_loss) / epoch_step,
                (val_total_triple_loss + val_total_CE_loss) / epoch_step_val)))


def epochTrain_arcface(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen,
                       gen_val,
                       endEpoch, Batch_size, scaler, save_period, save_dir, flag):
    # 交叉熵损失
    total_loss = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        optimizer.zero_grad()
        with autocast():
            outputs = model_train(images, "train")
            _loss = nn.NLLLoss()(F.log_softmax(outputs, dim=-1), labels)
        # 反向传播
        scaler.scale(_loss).backward()
        # 参数优化
        scaler.step(optimizer)
        ...
        if flag == 0:
            pbar.set_postfix(**{'CE_loss': total_loss / (iteration + 1),
                                'accuracy': total_accuracy / (iteration + 1),
                                'lr': get_lr(optimizer)})
            pbar.update(1)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            accuracy = torch.mean((torch.argmax(F.softmax(outputs, dim=-1), dim=-1) == labels).type(torch.FloatTensor))
            val_total_loss += _loss.item()
            val_total_accuracy += accuracy.item()

        if flag == 0:
            pbar.set_postfix(**{'val_CE_loss': val_total_loss / (iteration + 1),
                                'val_accuracy': val_total_accuracy / (iteration + 1),
                                'lr': get_lr(optimizer)})
            pbar.update(1)
    ...
    if flag == 0:
        ...
        if (epoch + 1) % save_period == 0 or epoch + 1 == endEpoch:
            torch.save(model.state_dict(), os.path.join(save_dir, 'arcface_%03d-l%.3f-vl%.3f.pth' % (
